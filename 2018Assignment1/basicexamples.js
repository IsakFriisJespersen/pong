...
function drawRectsObservable() {
    const svg = document.getElementById("drawRects"), mousemove = Observable.fromEvent(svg, 'mousemove'), mouseup = Observable.fromEvent(svg, 'mouseup'), svgRect = svg.getBoundingClientRect();
...
function drawAndDragRectsObservable() {
    const svg = document.getElementById("drawAndDragRects"), mousemove = Observable.fromEvent(svg, 'mousemove'), mouseup = Observable.fromEvent(svg, 'mouseup'), svgRect = svg.getBoundingClientRect();
    Observable.fromEvent(svg, 'mousedown')
        .map(({ clientX, clientY }) => ({ x0: clientX - svgRect.left, y0: clientY - svgRect.top, rect: new Elem(svg, 'rect'), }))
        .flatMap(({ rect, x0, y0 }) => mousemove
        .takeUntil(mouseup)
