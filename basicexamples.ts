...
function animatedRect() {
  ...
  const svg = document.getElementById("animatedRect")!; // skapar en svg


  let rect = new Elem(svg, 'rect') // skapar en instans till rektangeln
    .attr('x', 50).attr('y', 10) // sätter x och y koordinater alltså vart i canvasen den ska starta
    .attr('width', 120).attr('height', 80) // skapar bredd och höjd
    .attr('fill', '#95B3D7'); // fyller den med färg
...
function drawRectsEvents() {
  const svg = document.getElementById("drawRects")!; // hämtar canvas
...
function drawRectsObservable() {
  // implement this function!
    ...
    Observable.fromEvent<MouseEvent>(svg, 'mousedown')
        .map(({clientX, clientY}) => ({ x0: clientX - svgRect.left, y0: clientY - svgRect.top, rect: new Elem(svg, 'rect'),}))
        .flatMap(({rect, x0, y0}) =>
            mousemove
                .takeUntil(mouseup)
                .map(({clientX, clientY}) => ({x1:clientX - svgRect.left, y1:clientY - svgRect.top, rect:rect}))
                .map(({x1,y1}) => ({left:Math.min(x0, x1), top:Math.min(y0, y1), width:Math.abs(x0 - x1), height:Math.abs(y0 - y1), rect})))
        .subscribe(({left, top, width, height, rect})=> rect
                .attr('x', String(left))
                .attr('y', String(top))
                .attr('width', String(width))
                .attr('height', String(height))
